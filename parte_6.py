...
while jogando:
    ...
    if x_jogador != x_porta or y_jogador != y_porta:
        posica_monstro = bussola(x_monstro, y_monstro, x_jogador, y_jogador) 
        if posica_monstro == 'norte':
            y_monstro = y_monstro - 1 
        if posica_monstro == 'sul':
            y_monstro = y_monstro + 1  
        if posica_monstro == 'leste':
            x_monstro = x_monstro + 1  
        if posica_monstro == 'oeste':
            x_monstro = x_monstro - 1
    ...
    elif x_jogador != x_monstro or y_jogador != y_monstro:
        resultado_dica = dica(x_monstro, y_monstro, x_jogador, y_jogador)
        print(resultado_dica)

    # TODO 3 Adicione abaixo o código referente ao exercício: Verifica se o Jogador encontrou a porta

    while (x_jogador != x_porta or y_jogador != y_porta) and (x_jogador != x_monstro or y_jogador != y_monstro):
        tabuleiro = imprime_tabuleiro(x_monstro, y_monstro, x_jogador, y_jogador, x_porta, y_porta, largura, altura, debug)
        print(tabuleiro)
    ...
    if (x_jogador == x_porta and y_jogador == y_porta) and (x_jogador != x_monstro and y_jogador != y_monstro): 
        print("Achou a porta!")
